...
class SimpleCNN(object): 
    def __init__(self, input, phase, dropout, num_classes, skip_layer, weights_path='DEFAULT',config = None):
        """
        ...
        if self.config.dataset == 'mnist':
            self.create_graph()
            print('Completed constructing graph of SimpleCNN for mnist.')
        elif self.config.dataset == 'cifar100':
            self.creat_graph_cifar100()
            print('Completed constructing graph of SimpleCNN for cifar100.')


    def creat_graph_cifar100(self):
        # First layer
        # Reshape the images to [BATCH_SIZE, 32, 32, 1] to make it work with tf.nn.conv2d
        images = tf.reshape(self.X, shape=[-1, 32, 32, 3])
